...
本机缓存已确认领取后，不能更换手机号重复领取；后端也会按活动周期限制同一 IP 只允许一个手机号领取，当前活动周期为 3 个月。
...
- SKILL.md frontmatter 只保留 name 和 description，保证 Codex/Claude 类运行时轻量识别
...
- `skill/skill.json` 声明运行时、安装方式、网络和隐私边界；`SKILL.md` 只保留 Agent 必读规则。
...
- 用户订单问题走 `/skill/xinyi/orders` 专用订单接口，覆盖全部订单、历史订单/已完成订单和正在进行中订单；订单信息必须以接口返回为准，不能预估、估算或模糊处理；订单评级由脚本按接口返回杯数计算；“我定了哪些饮料/喝了多少咖啡”只是基于订单数据的回答方式。
- 服务端接口返回结构化原始数据；Skill 脚本会再整理成文本/表格给大模型使用。
- 活动和手机号领取严格以接口结果为准；门店、菜单、推荐等读类能力在接口失败时会输出可读降级提示，避免编造实时数据。
- 用户参与活动或查询活动状态并拿到可判定接口结果后，手机号和活动状态会默认保存在本地状态文件中；普通推荐、门店和菜单查询不会自动复用缓存手机号。
...
- 服务端领取接口会读取请求 IP 做活动周期风控；同一 IP 在 3 个月活动周期内只允许一个手机号领取，未注册或接口失败不会占用 IP 领取名额；同一 IP 10 分钟内领取尝试过多会被限流。
...
| Registry 元数据 | `skill/skill.json` 中的 `openclaw` 与运行时声明 |
...
| `POST /skill/xinyi/claim` | POST | 手机号会作为请求数据发送到后端，JSON 形如 `{"mobile":"..."}`；服务端会读取请求 IP 做 3 个月活动周期风控 | 查询并领取活动奖励 |
...
- `skill/SKILL.md`: Agent 使用规则；frontmatter 只保留 name 和 description
...
- `skill/scripts/query_orders.py`: 调用 `/skill/xinyi/orders`，查询全部订单、历史订单/已完成订单和正在进行中订单，并可基于返回数据输出已定饮品、可见杯数、订单评级、可确认咖啡相关杯数等摘要；订单信息必须以接口返回为准，不能预估、估算或模糊处理；订单接口失败时不猜用户下过几单
- `skill/scripts/recommend_drink.py`: 调用 `/skill/xinyi/context`，并把接口返回的商品、天气和可选订单历史整理成推荐上下文表格/文本；默认不读取本地缓存手机号，活动总览或明确个性化推荐才由 Agent 内部显式复用缓存；接口失败时保留品牌流程和推荐思路，不编造菜单或门店
