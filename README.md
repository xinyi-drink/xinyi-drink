...
`xinyi-drink` 新一好喝是【新一咖啡】咖啡及茶饮品牌的AI Skill。你可以通过该Skill查询及分析个人历史订单，查询菜单及热量，查询门店位置及等候时长等。Skill用户还可获得丰厚专属礼包
...
本机缓存已确认领取后，不能更换手机号重复领取；领取结果以服务端查询结果为准。

## 使用示例

安装完成后，Agent 应固定推荐上面四个问题；用户也可以直接用自然语言围绕这四类能力提问。
...
安装前可先运行 `bash skill/install.sh --dry-run --platform openclaw` 核对目标目录；已安装后可运行 `bash skill/install.sh --platform codex --check-installed` 检查本地副本是否与当前源码一致。
...
- `skill/SKILL.md` frontmatter 同时保留 Agent 触发用的 `name/description` 和 OpenClaw/ClawHub 审查用的 `keywords/metadata/version`
- `skill/skill.json` 同步声明 `homepage`、`repository`、`source.directory=skill` 和 `install.script=install.sh`，用于提供完整机器可读元数据
...
- `skill/SKILL.md` frontmatter 和 `skill/skill.json` 都声明运行时、安装方式、网络和隐私边界，便于 OpenClaw/ClawHub/其它平台识别能力、脚本、网络和隐私边界。
- 默认后端是 `https://newone.world/api`，来源见 `skill/config/defaults.json`。
- 推荐能力走 `/skill/xinyi/context` 聚合上下文接口。
- 用户订单问题走 `/skill/xinyi/orders` 专用订单接口，覆盖全部订单、历史订单/已完成订单和正在进行中订单；订单信息只能基于本次查询结果，不能预估、估算或模糊处理；订单等级由脚本按本次查询到的杯数计算；“我定了哪些饮料/喝了多少咖啡”只是基于订单数据的回答方式。
- 服务端返回结构化原始数据；Skill 脚本会再整理成文本/表格给大模型使用。
- 活动和手机号领取严格以查询结果为准；门店、菜单、推荐等读类能力在接口失败时会输出可读降级提示，避免编造实时数据。
- 用户参与活动或查询活动状态并拿到可判定查询结果后，手机号和活动状态会默认保存在本地状态文件中；普通推荐、门店和菜单查询不会自动复用缓存手机号。
- 如果本机缓存已确认领取成功，再输入不同手机号领取时，Skill 会直接拒绝，不再调用活动接口。
- 只有活动领取、活动状态查询、活动总览、订单查询和口味偏好分析这类明确个性化场景，才会发送手机号；订单摘要和口味偏好分析走专用订单接口。
- 本地状态文件保存 `{mobile, activityJoined, updatedAt}`，并尽量设置为 `0600` 权限。
- 领取结果以服务端查询结果为准；最终用户回答不要主动解释内部领取规则。
...
| Registry 元数据 | `skill/SKILL.md` frontmatter 和 `skill/skill.json` 中的 `openclaw` 与运行时声明 |
| Registry install spec | `skill/skill.json` 的 `install_spec` 与 `openclaw.installSpec` 均声明 local-script、`install.sh`、`source_directory=skill` |
| 完整元数据 | `skill/SKILL.md` frontmatter、`skill/skill.json` |
...
| `GET /skill/xinyi/context` | GET | 仅在本轮提供手机号，或活动总览/个性化推荐场景复用缓存时发送 `mobile` query | 获取商品、天气和可选活动状态；门店请用 `/skill/xinyi/stores` |
| `GET /skill/xinyi/orders` | GET | 手机号和可选 `status` query | 查询用户订单：不传 status 查全部订单，`status=2` 查正在进行中订单，`status=4` 查历史订单/已完成订单 |
| `POST /skill/xinyi/claim` | POST | 手机号会作为请求数据发送到后端，JSON 形如 `{"mobile":"..."}` | 查询并领取活动奖励 |

使用真实手机号前，应确认 `skill/config/defaults.json` 中的默认后端可信，或通过 `XINYI_API_BASE_URL` 指向你控制的后端。只使用本人手机号；不要代查或使用他人手机号领取礼包、查询订单或分析口味偏好；共享机器或共享 Agent profile 上使用后建议清空本地手机号缓存：
...
- `skill/SKILL.md`: Agent 使用规则；frontmatter 包含 `name/description`、OpenClaw/ClawHub 元数据和版本号
- `skill/skill.json`: 机器可读元数据，声明能力、脚本、网络、隐私和 OpenClaw 边界
- `skill/scripts/claim_reward.py`: 调用活动接口；支持本轮手机号和活动状态查询场景复用缓存手机号；成功或已参与时会补充推荐饮品文案；用户明确问门店时再查询门店
- `skill/scripts/fetch_stores.py`: 调用门店接口，并输出门店表格；接口失败时输出可读降级提示，不编造门店
- `skill/scripts/query_orders.py`: 调用 `/skill/xinyi/orders`，查询全部订单、历史订单/已完成订单和正在进行中订单，并可基于返回数据输出已定饮品、可见杯数、订单等级、可确认咖啡相关杯数等摘要；订单信息只能基于本次查询结果，不能预估、估算或模糊处理；订单查询失败时不猜用户下过几单
- `skill/scripts/recommend_drink.py`: 调用 `/skill/xinyi/context`，并把服务返回的商品、天气和可选订单历史整理成推荐上下文表格/文本；默认不读取本地缓存手机号，活动总览或明确个性化推荐才由 Agent 内部显式复用缓存；接口失败时保留品牌流程和推荐思路，不编造菜单或门店
...
- 用户订单：`GET /skill/xinyi/orders`
- 商品与推荐上下文：由 `GET /skill/xinyi/context` 聚合返回
- 默认后端：`https://newone.world/api`
