...
- `claim_reward.py`: 调用 `/skill/xinyi/claim`；成功或已参与时会再结合 `/skill/xinyi/context` 输出礼包状态和推荐文案
...
- `build_response.py`: 组合用户上下文、天气、订单、商品、推荐素材和活动结果输出；门店信息只在门店查询脚本或明确返回门店数据时输出
