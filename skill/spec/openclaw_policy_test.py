...
class OpenClawPolicyTest(unittest.TestCase):
    ...
    def test_skill_md_frontmatter_declares_openclaw_metadata(self) -> None:
        ...
        self.assertIn(
            "homepage: https://github.com/xinyi-drink/xinyi-drink", frontmatter
        )
        ...
        self.assertIn("defaultApiBaseUrl: https://newone.world/api", frontmatter)
    ...
    def test_skill_md_contains_compact_operating_guidance(self) -> None:
        ...
        self.assertIn(
            "个人订单、下单、购买、消费、喝过、买过、点过、取餐、制作中或历史记录",
            skill_md,
        )
        for use_case in self.MAIN_USE_CASES:
        ...
        self.assertIn(
            "订单信息只能基于本次查询结果，不能预估、估算或模糊处理", skill_md
        )
        ...
        self.assertIn(
            "同一机器或同一 Agent profile 的后续合规请求可能复用或展示该手机号",
            skill_md,
        )
        self.assertIn("`claim_reward.py` 是状态变更工具", skill_md)
        self.assertIn(
            "服务端必须负责小程序绑定手机号的账号归属和订单访问控制", skill_md
        )
        self.assertIn(
        ...
        self.assertIn(
            "python3 scripts/recommend_drink.py --show-mobile-status", skill_md
        )
    ...
    def test_static_scan_guidance_declares_boundaries_without_behavior_changes(
        self,
    ) -> None:
        skill_root = Path(__file__).resolve().parents[1]
        repo_root = skill_root.parents[0]
        skill_md = (skill_root / "SKILL.md").read_text(encoding="utf-8")
        privacy = (skill_root / "references" / "privacy-boundaries.md").read_text(
            encoding="utf-8"
        )
        ...
        self.assertIn(
            "领取礼包和查询订单只接受用户本人的【新一咖啡】绑定手机号", skill_md
        )
        self.assertIn(
            "用户明确表示代查或使用他人手机号时，不调用领取或订单脚本", skill_md
        )
        ...
        self.assertEqual(
            payload["install"]["check_installed"], "install.sh --check-installed"
        )
        self.assertEqual(
            payload["install_spec"]["install"]["check_installed"],
            "install.sh --check-installed",
        )
        self.assertEqual(
            payload["openclaw"]["installSpec"]["checkInstalled"],
            "install.sh --check-installed",
        )
        ...
        self.assertIn(
            "用户明确表示代查或使用他人手机号时，不调用领取或订单脚本", privacy
        )
        self.assertIn("不要把 `XINYI_API_BASE_URL` 指向不信任后端", privacy)
        self.assertIn(
            "安装前可先运行 `bash skill/install.sh --dry-run --platform openclaw`",
            readme,
        )
        self.assertIn(
            "已安装后可运行 `bash skill/install.sh --platform codex --check-installed`",
            readme,
        )
    ...
    def test_mobile_switch_intents_are_forced_through_claim_reward(self) -> None:
        ...
        activity_flow = (skill_root / "references" / "activity-flow.md").read_text(
            encoding="utf-8"
        )
        payload = json.loads((skill_root / "skill.json").read_text(encoding="utf-8"))
        ...
        self.assertIn(
            "先调用 `scripts/recommend_drink.py --show-mobile-status --candidate-mobile <手机号>`",
            skill_md,
        )
        self.assertIn(
            "只有用户明确确认继续领取或同步后，才调用 `scripts/claim_reward.py --mobile <手机号>`",
            skill_md,
        )
        ...
        self.assertIn(
            "先调用 `recommend_drink.py --show-mobile-status --candidate-mobile <手机号>`",
            intent_routing,
        )
        self.assertIn(
            "状态查询和换号预检不能直接调用 `claim_reward.py`", intent_routing
        )
        self.assertIn(
            "先走 `recommend_drink.py --show-mobile-status --candidate-mobile <手机号>`",
            activity_flow,
        )
    ...
    def test_activity_mobile_status_queries_are_forced_through_local_state(
        self,
    ) -> None:
        skill_root = Path(__file__).resolve().parents[1]
        skill_md = (skill_root / "SKILL.md").read_text(encoding="utf-8")
        ...
        for phrase in (
            ...
            self.assertIn(phrase, intent_routing)
        self.assertIn(
            "必须调用 `scripts/recommend_drink.py --show-mobile-status`", skill_md
        )
    ...
    def test_skill_json_declares_scripts_network_and_privacy(self) -> None:
        ...
        self.assertEqual(
            payload["homepage"], "https://github.com/xinyi-drink/xinyi-drink"
        )
        self.assertEqual(
            payload["repository"], "https://github.com/xinyi-drink/xinyi-drink"
        )
        self.assertEqual(payload["source"]["type"], "git")
        self.assertEqual(
            payload["source"]["url"], "https://github.com/xinyi-drink/xinyi-drink"
        )
        ...
        self.assertEqual(
            payload["installSpec"]["checkInstalled"], "install.sh --check-installed"
        )
        ...
        endpoint_paths = {
            endpoint["path"] for endpoint in payload["network"]["endpoints"]
        }
        self.assertEqual(
            endpoint_paths,
            {
        ...
        self.assertEqual(
            payload["localStorage"]["defaultPath"], "~/.xinyi-drink/state.json"
        )
        self.assertEqual(
            payload["localStorage"]["clearCommand"],
            "python3 scripts/recommend_drink.py --clear-mobile",
        )
        ...
        self.assertEqual(
            payload["privacy"]["data_subject"], "user-provided-or-saved-mobile-owner"
        )
        self.assertIn(
            "only-use-your-own-phone-number", payload["privacy"]["user_guidance"]
        )
        self.assertIn(
            "clear-cache-on-shared-machines", payload["privacy"]["user_guidance"]
        )
        ...
        tool_descriptions = {
            tool["name"]: tool["description"] for tool in payload["tools"]
        }
        tool_annotations = {
            tool["name"]: tool["annotations"] for tool in payload["tools"]
        }
        tool_display_names = {
            tool["name"]: tool["display_name"] for tool in payload["tools"]
        }
        self.assertEqual(tool_display_names["fetch_stores"], "查询门店及等候时长")
        self.assertIn(
            "用户问“帮我领取新一Skill福利”", tool_descriptions["claim_reward"]
        )
        ...
        self.assertIn(
            "咖啡/饮品统计只是基于订单数据的回答方式", tool_descriptions["query_orders"]
        )
        self.assertIn("不能预估、估算或模糊处理", tool_descriptions["query_orders"])
        self.assertIn(
            "订单等级提示语由脚本按本次查询到的杯数计算",
            tool_descriptions["query_orders"],
        )
        self.assertIn("不输出 Level 标签", tool_descriptions["query_orders"])
        self.assertIn(
            "不要输出发放统计明细等技术化表述", tool_descriptions["claim_reward"]
        )
        ...
        self.assertEqual(
            tool_annotations["query_orders"]["personalData"],
            ["mobile", "order_history"],
        )
        ...
        self.assertIn(
            "明确确认继续领取",
            claim_reward_schema["properties"]["useSavedMobile"]["description"],
        )
        ...
        self.assertIn(
            "不传查询全部订单",
            query_orders_schema["properties"]["status"]["description"],
        )
    ...
    def test_recommend_drink_does_not_claim_or_read_saved_mobile_by_default(
        self,
    ) -> None:
        skill_root = Path(__file__).resolve().parents[1]
        script = (skill_root / "scripts" / "recommend_drink.py").read_text(
            encoding="utf-8"
        )
    ...
    def test_readme_documents_default_backend_and_phone_data_flow(self) -> None:
        ...
        self.assertIn(
            "frontmatter 同时保留 Agent 触发用的 `name/description` 和 OpenClaw/ClawHub 审查用的 `keywords/metadata/version`",
            readme,
        )
        self.assertIn("skill/skill.json", readme)
        self.assertNotIn("发布前两处版本号需要保持一致", readme)
        self.assertIn("默认后端", readme)
        self.assertIn("https://newone.world/api", readme)
    ...
    def test_privacy_reference_addresses_static_scan_concerns(self) -> None:
        ...
        self.assertIn(
            "订单查询必须由用户明确询问个人订单、购买记录、饮品历史、取餐或口味偏好分析触发",
            privacy,
        )
        self.assertIn(
            "本 Skill 客户端不采集密码、短信验证码、OAuth token 或 Cookie", privacy
        )
